...
function conpoundCheck(){
  ...
  number *= 3; // number * 3
  console.log(`number *= 20 : ${number}`);//180
...
/** AND(&&) 둘다 true 인경우에만 true (나머진 false) */  



/** AND 연산자 확인*/
function andcheck(){
  // 104는 100 이상의 숫자이면서 짝수 인가?

 const bool1 = (100 <= 104) && (104 % 2 === 0 );
 console.log(`104는 100 이상의 숫자이면서 짝수 인가? ${bool1}`);
...
function orCheck(){
  ...
  console.log(`4는 10을 초과하거나 짝수이다 ${bool1}`);


  const name = "홍길동";
  const age = 20;
  //나이가 18세 미만 이거나 이름이 "김길동인 경우"
  const bool2 = (18 >= 20 ) || ( "홍길동" === "김길동");
  console.log(`나이가 18세 미만 이거나 이름이 "김길동인 경우 ${bool2}`);
